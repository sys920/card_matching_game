...
function checkMach() {      //Checking 2Cards match or unmatch 
    if ((tempArray[0][1] == tempArray[1][1])) {  
        cards[tempArray[0][0]].setAttribute('class','card match effect');  //1st, 2nd card class were matched
        cards[tempArray[1][0]].setAttribute('class','card match effect'); 

        tempArray = [];
        checkFlipCount2more = 0;   
        if (endingcheck.length == 16 ) {        //Checking 16Cards match and showing winner msg. 
            
            for (const card of cards) {     //Effecting matched cards 
                card.setAttribute('class','card match rotate');     //If 2 cards match, open cards: changing of class   
            }
        ...
        cards[tempArray[0][0]].setAttribute('class','card');        //If 2 cards do not match, close cards 
        cards[tempArray[1][0]].setAttribute('class','card');
